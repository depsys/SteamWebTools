...
function inventoryPageInit(){
	...
	if (window.UserYou.strSteamId!=window.g_steamID){
		...
		window.PopulateMarketActions = function (elActions, item) {
			...
			$('#iteminfo0_item_name').before('<div class="descriptor"><br/>ClassID#: <span style="color:green">'+item.classid+'</span> Defindex#: <span style="color:red">'+item.app_data.def_index+'</span><br/></div>');
			$('#iteminfo1_item_name').before('<div class="descriptor"><br/>ClassID#: <span style="color:green">'+item.classid+'</span> Defindex#: <span style="color:red">'+item.app_data.def_index+'</span><br/></div>');
			if (!item.marketable) {
				return res;
			}
